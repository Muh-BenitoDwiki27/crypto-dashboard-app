...
const SearchItem = ({ coin, onSelect, isActiveName }: SearchItemProps) => {
    ...
    const isSearchCoin = 'data' in coin && typeof coin.data?.price_change_percentage_24h === 'number';

    let change = 0;

    if (isSearchCoin) {
        change = (coin as SearchCoin).data?.price_change_percentage_24h ?? 0;
    } else {
        const trendingCoin = coin as TrendingCoin['item'];
        change = trendingCoin.data?.price_change_percentage_24h?.usd ?? 0;
    }
    ...
    return (
        <CommandItem value={coin.id} onSelect={() => onSelect(coin.id)} className='search-item'>
            ...
            <div className={cn('coin-change', {'text-green-500': change > 0, 'text-red-500': change < 0,})}>
                {change > 0 ? (
...
export const SearchModal = ({ initialTrendingCoins = [], }: { initialTrendingCoins: TrendingCoin[]; }) => {
    ...
    useKey((event) => event.key?.toLowerCase() === 'k' && (event.metaKey || event.ctrlKey),
        (event) => {
        ...
        <div id="search-modal">
            ...
            <CommandDialog open={open} onOpenChange={setOpen} className='dialog' data-search-modal>
                <div className='cmd-input'>
                    <CommandInput placeholder='Search for a token by name or symbol...' value={searchQuery} onValueChange={setSearchQuery} />
                </div>
